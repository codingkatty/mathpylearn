...
function updateProgressBar() {
  ...
  if (lessonProgress) {
    const completedLessons = Object.values(userProgress.lessons).filter(
      Boolean
    ).length;
    ...
    if (lessonResetBtn) {
      lessonResetBtn.style.display =
        completedLessons > 0 ? "inline-block" : "none";
    }
  }

  if (challengeProgress) {
    const completedChallenges = Object.values(userProgress.challenges).filter(
      Boolean
    ).length;
    const totalChallenges =
      document.querySelectorAll("#challenge-list li").length;
    ...
    if (challengeResetBtn) {
      challengeResetBtn.style.display =
        completedChallenges > 0 ? "inline-block" : "none";
    }
  }
}
...
my_list.pop()
print("Removing the last element:", my_list)  # Output: [1, 20, 4, 5]`,
      1
    );
...
document.querySelectorAll("#challenge-list a").forEach((link) => {
  link.addEventListener("click", function (e) {
    ...
    document
      .querySelectorAll("#challenge-list a")
...
function openMobileMenu() {
  ...
  // Check if navLinks are currently displayed
  if (navLinks.style.display === "flex" || navLinks.style.display === "") {
    // Hide navLinks and show menuIcon
    navLinks.style.display = "none";
    menuIcon.style.display = "inline-block";
    closeIcon.style.display = "none";
  } else {
    // Show navLinks and hide menuIcon
    navLinks.style.display = "flex";
